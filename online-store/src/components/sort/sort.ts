...
export function SortProductCard(trigger: string): void { // 'now' для выпослнения сразу после фильтрации
  ...
  function update() {
    let sortArrproducts: Array<productsDataI> = [];
    if (resultData.length === 0 && (document.querySelector('.found') as HTMLElement).dataset.found === '0') sortArrproducts = productsData.products.slice()
    ...
    if ((document.querySelector('.found') as HTMLElement).dataset.found !== '0') CreateProductCard(sortArrproducts) // вызов функции создания карточек
  }
